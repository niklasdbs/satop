...
class FullObservationPOTOPEncoder : public ObservationEncoder{
public:
    FullObservationPOTOPEncoder(Graph& graph, int number_of_resources, int number_of_actions, Config config);
...
private:
    ...
    std::tuple<py::array_t<float>, py::array_t<float>>  encode_resource_history(TOPEnvState& state, Agent& current_agent);
};